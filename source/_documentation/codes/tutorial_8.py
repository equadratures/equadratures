...
def sparse():
    ...
    tensor = Basis('sparse-grid', level=7, growth_rule='exponential')
    myPoly = Poly([x1, x2], tensor, method='numerical-integration')
    myPoly.set_model(function)
    ...
    plt.xticks(fontsize=13)
    plt.yticks(fontsize=13)
    plt.savefig('../Figures/tutorial_8_fig_c.png' , dpi=200, bbox_inches='tight', pad_inches=0.1)
    ...
    cbar = plt.colorbar(extend='neither', spacing='proportional',
                orientation='vertical', shrink=0.8, format="%.0f")
    cbar.ax.tick_params(labelsize=13)
    plt.savefig('../Figures/tutorial_8_fig_d.png',   dpi=200, bbox_inches='tight')
