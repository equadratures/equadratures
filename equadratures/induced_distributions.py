...
from equadratures.parameter import Parameter, getlocalquadrature
...
class InducedSampling:
    ...
    def inverse_induced_jacobi(self, n):
        ...
        for q in range(0, n):
            ...
            ug, exponents = self.fast_induced_jacobi_distribution_setup_helper_1(ug, exps)

            def idistinv(uu):
                x = self.induced_distribution_jacobi_bisection(uu, nn, alpha, beta)
                return x

            data[nn] = self.fast_induced_jacobi_distribution_setup_helper_2(ug, idistinv, exponents, M)
    ...
    def induced_jacobi_distribution(self, x, n, M=None):
        ...
        mrs_centroid = self.median_approximation_jacobi(alph, bet, n)
        ...
        for xq in range(0, len(x)):
            ...
            for j in range(1, n+1):
                ab = self.quadraticModification(ab, un[j])
            ...
            for aq in range(0, int(A)):
                ab = self.linearModification(ab, root)
                logfactor += logfactor + np.log(ab[0, 1] * 1.0/2.0 * (x[xq]+1.0))
                ab[0, 1] = 1.0

            # M-point Gauss quadrature for evaluation of auxilliary integral integral
            u, w = self._getLocalQuadrature(M)
            integral = np.dot(w, (2.0 - 1.0/2.0 * (u+1.) * (x[xq]+1.))**Aa)
            F[xq] = np.exp(logfactor-alph*np.log(2.0)-beta(bet+1.0, alph+1.0)-np.log(bet+1.0)+(bet+1)*np.log((x[xq]+1.0)/2.0))*integral
        return F
    ...
    def induced_distribution_jacobi_bisection(self, u, n, alpha, beta):
        ...
        if n == 1:
            ...
            def primitive(x):
                value = self.induced_jacobi_distribution(x, n)
                return value
    ...
    def linearModification(self, ab, x0):
        """
        ...
        r = np.reshape(np.abs(self.evaluateRatioSuccessiveOrthoPolynomials(alpha, beta, x0, N-1)), (length_alpha - 1, 1))
        ...
        for i in range(0, N-1):
            ...
            ab[i, 0] = alpha[i] + np.sign * acorrect[i]

        return ab

    def quadraticModification(self, alphabeta, x0):
        """
        ...
        alpha = alphabeta[:, 0]
        beta = alphabeta[:, 1]
        C = np.reshape(self.christoffelNormalizedOrthogonalPolynomials(alpha, beta, x0, N-1), [N, 1])
